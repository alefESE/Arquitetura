...
void lea(char *op1, char *op2){
    ...
    *remove(op2, op2+strlen(op2), '\n') = 0;
    ...
    if((std::all_of(op2, op2+strlen(op2), ::isdigit))){ // b constante
        ...
        ptr = is_reg(op1, offset, i);
        if(offset == 32){
            if(!(elemento(mems, temp.ex, &var))){
                cout << "Este endereco nao existe." << endl;
                cout << "Abortando programa..." << endl;
                fechar();
                exit(EXIT_FAILURE);
            } else {
...
void str(char *op1, char *op2){
    ...
    *remove(op2, op2+strlen(op2), '\n') = 0;
    ...
    if((std::all_of(op1, op1+strlen(op1), ::isdigit))){ // b constante
        ...
        if(offset == 32){
            mem.seekg (0, mem.beg); //"rebobina" stream
            for(i = 0; i < temp.ex; i++){
                if(!(elemento(mems, i+2, &var))) insere(mems, i+2, 0, 0);
                getline(mem, out, '\n');
                 if(mem.eof()){
                     mem << "\n";
                     mem.clear();
            ...
            mem.seekg (0, mem.beg); //"rebobina" stream
            for(i = 0; i < temp.ex; i++){
                if(!(elemento(mems, i+1, &var))) insere(mems, i+1, 0, 0);
                 mem.getline(NULL, palavra, '\n');
                 if(mem.fail() && mem.eof()){
                     mem << endl;
                     mem.flush();
                     mem.clear();
